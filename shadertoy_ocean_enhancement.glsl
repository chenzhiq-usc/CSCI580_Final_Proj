...
float getWaveHeight(vec2 worldPos, float time) {
    ...
    height += noise2D(worldPos * 0.6 + time * vec2(0.2, 0.15)) * 0.25;  
    height += noise2D(worldPos * 1.0 + time * vec2(-0.15, 0.2)) * 0.15; 
    height += noise2D(worldPos * 2.5 + time * vec2(0.12, -0.12)) * 0.08;
    
    height = (height - 0.24) * 2.5; 
    height += 0.2; 
...
vec3 getWaterNormal(vec2 worldPos, float time, float distance) {
    // Increase water normals to highlight waves
    vec3 normalA = sampleWaterNormal(worldPos, time, 0.8, vec2(0.2, 0.15), 1.2, distance); 
    vec3 normalB = sampleWaterNormal(worldPos, time, 1.2, vec2(-0.15, 0.2), 1.2, distance); 
    vec3 normalC = sampleWaterNormal(worldPos, time, 2.0, vec2(0.12, -0.12), 0.9, distance); 
...
float calculateFoam(float depth, float time, vec2 worldPos) {
    float foamNoise = noise2D(worldPos * 5.0 + time * vec2(0.7, 0.5));
    
    float foam = step(0.65, foamNoise);
...
vec3 renderWater(vec3 ro, vec3 rd, vec3 skyColor) {
    ...
    // Far distane will be darker and more blue
    float depthDarkening = smoothstep(1.0, 10.0, t);
    ...
    vec3 shadedColor;
    if (diffuse > 0.7) {
        shadedColor = vec3(0.5, 0.8, 0.95);    
    } else if (diffuse > 0.4) {
        shadedColor = vec3(0.2, 0.5, 0.85);    
    } else {
        shadedColor = vec3(0.08, 0.3, 0.65); 
    }

    vec3 halfVec = normalize(lightDir + viewDir);
    float spec = pow(max(0.0, dot(normal, halfVec)), 20.0);
    if (spec > 0.6) {
        shadedColor = vec3(0.7, 0.85, 0.95); 
    }

    shadedColor = mix(shadedColor, shadedColor * 0.92, depthDarkening * 0.6);
    ...
    shadedColor = mix(shadedColor, vec3(0.95, 0.98, 1.0), foam);  
